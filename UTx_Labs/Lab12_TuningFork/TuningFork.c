...
unsigned long previousInput = 0;	//Remembers switch value from last ISR
unsigned char nextState = 0;			//Either 0 or 1 depending on input states
...
void Sound_Init(void)
{
	...
	//NVIC_ST_CTRL_ENABLE(bit 0)+NVIC_ST_CTRL_CLK_SRC(bit 1)+NVIC_ST_CTRL_INTEN(bit 2)
	NVIC_ST_CTRL_R = 0x07;
...
void SysTick_Handler(void)
{
		...
		if((previousInput == 0x00) && (input == 0x08))
			nextState ^= 1;	//Toggle next state
		
		if(nextState == 1)
			GPIO_PORTA_DATA_R ^= 0x04;	//Toggle PA2
		
		previousInput = input;	//Remember current ISR input value for later
