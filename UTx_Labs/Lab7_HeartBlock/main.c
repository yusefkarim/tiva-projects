...
{
	//write this function
	//Complete: Waits for PF4 to be low
	do
		SW1 = GPIO_PORTF_DATA_R&0x10;		//Read value from SW1(AS)
	while(SW1);		
}
...
{
	//write this function
	//Complete: Waits for PF4 to be high
	do
		SW1 = GPIO_PORTF_DATA_R&0x10;		//Read value from SW1(AS)
	while(!SW1);
}
...
{
	// write this function
	//Complete:Delays approx. 1 msec
	unsigned long i;
	while(msec > 0)
	{
		//Value for i was determined more or less by trial and error using oscilloscope
		//It varied greatly when using the debugger and lab software!
		i = 400000 / 60;
		while(i > 0)
			i--;
		msec--;
